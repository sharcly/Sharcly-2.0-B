...
class StatsService {
    ...
    static async getSalesAnalytics(range = "30days") {
        const now = new Date();
        ...
        const [currentMetrics, prevMetrics, ordersInPeriod, topProducts] = await Promise.all([
            // Current Period
            prisma_1.prisma.order.aggregate({
                where: {
                    status: client_1.OrderStatus.DELIVERED,
            ...
            prisma_1.prisma.orderItem.groupBy({
                by: ['productId'],
                where: { order: { createdAt: { gte: startDate, lte: endDate }, status: client_1.OrderStatus.DELIVERED } },
                _sum: { quantity: true },
        ...
        const productIds = topProducts.map(tp => tp.productId).filter((id) => id !== null);
        ...
        const recentTransactionsRaw = await prisma_1.prisma.order.findMany({
        ...
        const orderHours = ordersInPeriod.reduce((acc, order) => {
        ...
        const newCustomersCount = await prisma_1.prisma.user.count({
            where: { createdAt: { gte: startDate, lte: endDate }, userRole: { slug: "user" } }
        });
        const prevNewCustomersCount = await prisma_1.prisma.user.count({
        ...
        const totalOrdersInPeriod = await prisma_1.prisma.order.count({ where: { createdAt: { gte: startDate, lte: endDate } } });
        const successRate = totalOrdersInPeriod > 0 ? ((orderCount / totalOrdersInPeriod) * 100).toFixed(1) + "%" : "0%";
        // 10. Real Retention Rate
        const uniqueUsersThisPeriod = await prisma_1.prisma.order.groupBy({
            by: ['userId'],
            where: { createdAt: { gte: startDate, lte: endDate }, status: client_1.OrderStatus.DELIVERED }
        });
        let repeatUsers = 0;
        for (const u of uniqueUsersThisPeriod) {
            const pastOrders = await prisma_1.prisma.order.count({
                where: { userId: u.userId, createdAt: { lt: startDate }, status: client_1.OrderStatus.DELIVERED }
            });
            if (pastOrders > 0)
                repeatUsers++;
        }
        ...
        const cancelledOrders = await prisma_1.prisma.order.count({ where: { status: 'CANCELLED', createdAt: { gte: startDate, lte: endDate } } });
