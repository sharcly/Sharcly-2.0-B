...
const getAllPaymentGateways = async (req, res) => {
    try {
        const gateways = await admin_service_1.AdminService.getAllPaymentGateways();
...
const createPaymentGateway = async (req, res) => {
    try {
        const gateway = await admin_service_1.AdminService.createPaymentGateway(req.body);
...
const updatePaymentGateway = async (req, res) => {
    try {
        const { id } = req.params;
        const gateway = await admin_service_1.AdminService.updatePaymentGateway(id, req.body);
...
const deletePaymentGateway = async (req, res) => {
    try {
        const { id } = req.params;
        await admin_service_1.AdminService.deletePaymentGateway(id);
...
const resetPaymentGatewayCounter = async (req, res) => {
    try {
        const { id } = req.params;
        const gateway = await admin_service_1.AdminService.resetPaymentGatewayCounter(id);
        res.status(200).json({ success: true, gateway });
    }
    catch (error) {
        res.status(500).json({ message: error.message || "Failed to reset payment gateway counter" });
    }
};
exports.resetPaymentGatewayCounter = resetPaymentGatewayCounter;
