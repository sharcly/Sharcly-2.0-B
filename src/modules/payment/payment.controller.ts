...
export const oauthConnect = async (req: any, res: Response) => {
  ...
  const redirectUri = `${process.env.FRONTEND_URL || "http://localhost:3000"}/dashboard/payments?oauth=success&gateway=${gateway}`;
  ...
  const oauthSimulationUrl = `http://localhost:8181/api/payments/oauth/consent/${gateway}?userId=${userId}&redirect=${encodeURIComponent(redirectUri)}`;
...
export const oauthCallback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.redirect(`${process.env.FRONTEND_URL || "http://localhost:3000"}/dashboard/payments?oauth=failed&error=${encodeURIComponent(error.message)}`);
  }
};
